...
# Forward DC image/photo to TG (download -> sendPhoto for static, sendDocument for animated)
_sync_dc_photo_to_tg() {
	_raw="$1" _tcid="$2" _sender="$3"
	_chat="${_tcid%%/*}"
	_thr="${_tcid#*/}"
	[ "$_thr" = "$_chat" ] && _thr=""
	...
	for _att in $_items; do
		...
		case "$_ct" in
			image/*)
				_ts=$(date +%s)
				_tmp="/tmp/img/sync-dc-tg-$$-$_ts"
				http_get_file "$_url" "$_tmp" || { log_err "sync: dc->tg photo download FAIL"; rm -f "$_tmp"; continue; }
				if _sync_tg_multipart "$_chat" "$_thr" "$_tmp" "$_fn" "sendPhoto" "photo" "$_ct" "👾 $_sender: [图片]"; then
					_sent=$((_sent + 1)); log_info "sync: dc->tg photo OK"
				else
					log_err "sync: dc->tg photo FAIL: $_ERROR"
				fi
				rm -f "$_tmp"
				;;
		esac
...
# Forward DC file to TG
_sync_dc_file_to_tg() {
	_raw="$1" _tcid="$2" _sender="$3"
	_sync_dc_attachments_to_tg "$_raw" "$_tcid" "$_sender"
}

# Forward DC video to TG
_sync_dc_video_to_tg() {
	_raw="$1" _tcid="$2" _sender="$3"
	_chat="${_tcid%%/*}"
	_thr="${_tcid#*/}"
	[ "$_thr" = "$_chat" ] && _thr=""
	...
	for _att in $_items; do
		...
		case "$_ct" in
			video/*)
				_ts=$(date +%s)
				_tmp="/tmp/img/sync-dc-tg-$$-$_ts"
				http_get_file "$_url" "$_tmp" || { log_err "sync: dc->tg video download FAIL"; rm -f "$_tmp"; continue; }
				if _sync_tg_multipart "$_chat" "$_thr" "$_tmp" "$_fn" "sendVideo" "video" "$_ct" "👾 $_sender: [视频]"; then
					_sent=$((_sent + 1)); log_info "sync: dc->tg video OK"
				else
					log_err "sync: dc->tg video FAIL: $_ERROR"
				fi
				rm -f "$_tmp"
				;;
		esac
...
# Forward DC sticker to TG
_sync_dc_sticker_to_tg() {
	_raw="$1" _tcid="$2" _sender="$3"
	_chat="${_tcid%%/*}"
	_thr="${_tcid#*/}"
	[ "$_thr" = "$_chat" ] && _thr=""
	_items="$(json_get "$_raw" stickers 2>/dev/null)" || _items=""
	if [ -z "$_items" ] || [ "$_items" = "NOTFOUND" ]; then return 0; fi
	_ssplits="$(printf '%s' "$_items" | sed 's/},{/}\n{/g')"
	_sent=0
	IFS='
'
	for _st in $_ssplits; do
		_fmt="$(json_get "$_st" format 2>/dev/null)" || _fmt=""
		_url="$(json_get "$_st" url 2>/dev/null)" || _url=""
		[ "$_url" = "NOTFOUND" ] && continue
		[ -z "$_url" ] && continue
		_ts=$(date +%s)
		_tmp="/tmp/img/sync-dc-tg-stk-$$-$_ts"
		http_get_file "$_url" "$_tmp" || { log_err "sync: dc->tg sticker download FAIL"; rm -f "$_tmp"; continue; }
		# Static WEBM → sendPhoto, animated → sendDocument
		if [ "$_fmt" = "static" ]; then
			if _sync_tg_multipart "$_chat" "$_thr" "$_tmp" "sticker.webp" "sendPhoto" "photo" "image/webp" "👾 $_sender"; then
				_sent=$((_sent + 1)); log_info "sync: dc->tg sticker static OK"
			fi
		else
			if _sync_tg_multipart "$_chat" "$_thr" "$_tmp" "sticker.${_fmt}" "sendDocument" "document" "image/${_fmt}" "👾 $_sender"; then
				_sent=$((_sent + 1)); log_info "sync: dc->tg sticker ${_fmt} OK"
			fi
		fi
		rm -f "$_tmp"
	done
...
# Forward DC photo to QQ
_sync_dc_photo_to_qq() {
	_raw="$1" _gid="$2" _sender="$3"
	_atts="$(json_get "$_raw" attachments 2>/dev/null)" || _atts=""
	if [ -z "$_atts" ] || [ "$_atts" = "NOTFOUND" ]; then return 0; fi
	_items="$(printf '%s' "$_atts" | sed 's/},{/}\n{/g')"
	...
	for _att in $_items; do
		...
		case "$_ct" in
			image/*)
				_ts=$(date +%s)
				_tmp="/tmp/img/sync-dc-qq-$$-$_ts"
				http_get_file "$_url" "$_tmp" || { log_err "sync: dc->qq photo download FAIL"; rm -f "$_tmp"; continue; }
				_furi="file:///root/img/sync-dc-qq-$$-$_ts"
				_img_msg="[{\"type\":\"image\",\"data\":{\"uri\":\"$_furi\",\"summary\":\"👾 $_sender: [图片]\"}}]"
				if qq_message_send_group "$_gid" "$_img_msg" >/dev/null; then
					_sent=$((_sent + 1)); log_info "sync: dc->qq photo OK"
				fi
				rm -f "$_tmp"
				;;
		esac
...
# Forward DC file to QQ
_sync_dc_file_to_qq() {
	_raw="$1" _gid="$2" _sender="$3"
	_sync_dc_attachments_to_qq "$_raw" "$_gid" "$_sender"
}

# Forward DC video to QQ
_sync_dc_video_to_qq() {
	_raw="$1" _gid="$2" _sender="$3"
	_atts="$(json_get "$_raw" attachments 2>/dev/null)" || _atts=""
	if [ -z "$_atts" ] || [ "$_atts" = "NOTFOUND" ]; then return 0; fi
	_items="$(printf '%s' "$_atts" | sed 's/},{/}\n{/g')"
	...
	for _att in $_items; do
		...
		case "$_ct" in
			video/*)
				_ts=$(date +%s)
				_tmp="/tmp/img/sync-dc-qq-$$-$_ts"
				http_get_file "$_url" "$_tmp" || { log_err "sync: dc->qq video download FAIL"; rm -f "$_tmp"; continue; }
				_furi="file:///root/img/sync-dc-qq-$$-$_ts"
				_vid_msg="[{\"type\":\"video\",\"data\":{\"uri\":\"$_furi\"}}]"
				if qq_message_send_group "$_gid" "$_vid_msg" >/dev/null; then
					_sent=$((_sent + 1)); log_info "sync: dc->qq video OK"
				fi
				rm -f "$_tmp"
				;;
		esac
...
# Forward DC record to QQ
_sync_dc_record_to_qq() {
	_raw="$1" _gid="$2" _sender="$3"
	_atts="$(json_get "$_raw" attachments 2>/dev/null)" || _atts=""
	if [ -z "$_atts" ] || [ "$_atts" = "NOTFOUND" ]; then return 0; fi
	_items="$(printf '%s' "$_atts" | sed 's/},{/}\n{/g')"
	...
	for _att in $_items; do
		...
		case "$_ct" in
			audio/*)
				_ts=$(date +%s)
				_tmp="/tmp/img/sync-dc-qq-$$-$_ts"
				http_get_file "$_url" "$_tmp" || { log_err "sync: dc->qq record download FAIL"; rm -f "$_tmp"; continue; }
				_furi="file:///root/img/sync-dc-qq-$$-$_ts"
				_rec_msg="[{\"type\":\"record\",\"data\":{\"uri\":\"$_furi\"}}]"
				if qq_message_send_group "$_gid" "$_rec_msg" >/dev/null; then
					_sent=$((_sent + 1)); log_info "sync: dc->qq record OK"
				fi
				rm -f "$_tmp"
				;;
		esac
